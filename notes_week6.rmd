...
lapply(dogs[cols], median, na.rm = TRUE) # missing values are very important and should be get rid of
# do not put bracket after median
```

`lapply()` always returns the result as a list.

"l" for **list** result. ###important!!!!!!!
...
"s" for **simplified** result. #simplify the result to a vector

Examples:
```{r}
sapply(dogs[cols], median, na.rm = TRUE) # store in a vector
# be careful that there is not 0 index in R
```
...
```{r}
x = c(1, 7, 9, 2, 5)
group = c("blue", "red", "blue", "green", "red") # should have the same length
...
```{r}
dogs = readRDS("~/Desktop/stat 33b/dogs.rds")
...
vapply(dogs[c("weight", "height")], median, 5100, na.rm = TRUE) # the third argument is an example of the output

# vapply(dogs[c("weight", "height")], median, "hi", na.rm = TRUE)

# vapply(dogs[c("weight", "height")], median, c(1, 2), na.rm = TRUE)
vapply(list(c(1, 2, 2), c(4, 5, 6)), median, 3.14)
1:9 + 9
```
...
apply(m, 1, sd) # 1 stands for the row (so the output has two elements)
...
Choosing an Apply Function # see the powerpoint
==========================
...
output = if (x != 10) {
   message("hi") # is equal to print or System.out.print
   42
} else {
   message("bye")
   8
...
```{r}
f = function(x1, x2 = x1+1, origin = TRUE) {
   if (origin)
      return (0)
   return (x1 + x2)
   sum((x1 - x2) ^2)
