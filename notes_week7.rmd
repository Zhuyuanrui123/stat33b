...
```{r}
square = function(x) { # more clear version
  if (!is.numeric(x))
    return (NA)
...
square = function(x) { # more concise version
...
tail(temp, -1)
is_hot = temp >= 100
c(FALSE, tail(is_hot, -1) != head(is_hot, -1))
```
...
change = c(FALSE, tail(is_hot, -1) != head(is_hot, -1)) # tail means to get rid of the first position in a vector 
# -1 means get rid of the first argument and keep the rest, -2 means get rid of the first two arguemtns and keep the rest etc
# 1 means keep the last as a result, 2 means keep the last two as a result
change
num_changes = sum(change)
if (num_changes %% 2 == 0) { # going up and going down all gives true, but count only as one heat wave
  num_changes / 2
} else {
  (num_changes + 1) / 2
}
...
create_dogs = function() { # dogs defined within the scope of "create_dogs", which is not the scope "getdogs" defined
  dogs = "hello"
}
...
parrot = 42
get_parrot() # local variable in a function has the priority
```
...
f() # no need to supply x as it is never used

message("hi")

f(message("hello")) # this message NEVER runs
```
...
f(message("hi")) # print statement never gets side effects in R, but we can use the force statements to force evaluation
```
...
mean_tol(c(1, 3, 5, 0.01, 0.2, NA), 0.5, na.rm = TRUE) # passed through "..."
