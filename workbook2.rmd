...
The first and third paths is relative path since it starts from a directory not the home directory. The second path is an absolute path as it starts from a home directory.
...
YOUR ANSWER GOES HERE:
```{r}
# the name of my home directory is called "yuanruizhu"
normalizePath("~") # the name of my home directory
length(list.files("~")) # find the number of files in the directory
```
...
which.min(dogs$height)
which.max(dogs$longevity)
dogs
```
...
YOUR ANSWER GOES HERE:
```{r}
vol = read.delim("volerup.txt") # read in the tables
names(vol) # the column names
nrow(vol) # the number of volcano eruptions
str(vol) # the class the types of the columns

# For instance, "Status" column contains categorical data, its type is "chr" or "character".
```
