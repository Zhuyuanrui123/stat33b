...
```{r}
to_kelvin = function(celsius = 0, fahrenheit = 32) {
  (fahrenheit - 32) * 9 / 5 + 273.15 + celsius
}
to_kelvin(celsius = 0)
to_kelvin(fahrenheit = 32)
to_kelvin(celsius = 10)
to_kelvin(fahrenheit = 42)
to_kelvin(celsius = 10, fahrenheit = 42) # if called with both arguments, it will give an incorrect result
```
...
to_kelvin = function(temperature, unit) {
  if (unit == 'celsius') temperature + 273.15 else if (unit == 'fahrenheit') (temperature - 32) * 5 / 9 + 273.15 else NA
}
to_kelvin(0, 'celsius')
to_kelvin(32, 'fahrenheit')
```
